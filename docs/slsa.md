...
**signed provenance**. Both phases are now implemented: releases from 1.0.2
carry L2 provenance, and from 1.0.3 the provenance is generated by an isolated
builder — **Build L3**.
...
**Both phases are implemented.** From 1.0.2 every release carries signed
provenance (L2); from 1.0.3 that provenance is generated and signed inside the
SLSA generator's reusable workflow, out of reach of the build steps — **Build
L3**. The starting position below is kept because it is the evidence the plan was
built on, and because the same commands are how you confirm a future release has
not silently stopped producing provenance.
...
| **Provenance generated outside user-controlled steps** | ✅ 1.0.3+ | `generator_generic_slsa3.yml`, `provenance` job |
...
GitHub validates a called workflow's *declared* permissions against the caller's
grant when the run starts, and the generator's `upload-assets` job declares
`contents: write` whether or not it runs. Granting only `contents: read` fails
the entire run before any job executes, with no logs and only "This run likely
failed because of a workflow file issue" to go on. That cost a rehearsal cycle
here; the workflow carries a comment so it does not look like over-permissioning.
...
**Rehearsing.** `release.yml` has a `workflow_dispatch` trigger that runs `build`
and `provenance` and skips publishing, so a change here can be exercised from a
branch without touching PyPI:
...
That is cheaper and more targeted than the TestPyPI rehearsal this plan
originally called for — the publish path is unchanged and already proven by
1.0.2, so what needs exercising is the provenance path.
...
publish. Publishing is ordered *after* provenance for exactly that reason: a
provenance failure stops the release before the irreversible step.
